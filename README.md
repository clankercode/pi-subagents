...
- **Dashboard UI integration** — `pi-agent-dashboard` users can see subagents in the dashboard: a footer badge shows running/completed counts, the `/subagents` command opens a table view of all subagent history with row actions (**View Result** opens the agent's streaming log in your default viewer; **Abort** stops a running agent; **Steer** sends a quick `Continue` nudge), and lifecycle events trigger automatic UI refresh. For steer-with-custom-text, open the **`/steer-subagent`** form (agent picker + message textarea) — row actions can't collect text. No React or SDK required — uses the dashboard's Extension UI Module System
...
**Roll up child usage** (`rollupChildUsage`, default `false`): when on, each subagent's token usage deltas (`input` / `output` / `cacheWrite`) are also added to every ancestor along `parentAgentId`, recursively across nested managers. Parent-facing totals (notifications, `get_subagent_result` stats, record `lifetimeUsage`) then include the subtree. This is **accounting visibility only** — child sessions still bill as their own provider API calls. Toggle via `/agents → Settings → Roll up child usage`.
