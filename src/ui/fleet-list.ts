...
export class FleetList {
  ...
  setUICtx(ui: FleetUICtx): void {
    ...
    this.tui = undefined;
    // Some UI contexts (e.g. headless test mocks) don't expose onTerminalInput —
    // skip key-capture there; the list still renders, just isn't keyboard-navigable.
    this.inputUnsub = typeof ui.onTerminalInput === "function"
      ? ui.onTerminalInput(data => this.handleKey(data))
      : undefined;
  }
  ...
  private renderAgentRow(rosterIndex: number, sel: number, record: AgentRecord, width: number, theme: Theme): string {
    ...
    // Prefer record.lifetimeUsage — it includes rolled-up child usage when
    // rollupChildUsage is on. Activity is own-session only and would undercount.
